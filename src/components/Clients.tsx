  ...
  return (
    <div className="flex items-center justify-center h-60 sm:max-h-60 px-4 mx-4 bg-white shadow-xl border border-gray-100 hover:scale-105 transition duration-300 ease-in-out filter grayscale hover:grayscale-0">
      <img
        src={imageUrl}
        alt={altText}
        className="max-h-20 sm:max-h-32 w-full object-contain"
      />
    </div>
  );
};
...
function ClientsRow({ clients }: { clients: any }) {
    ...
    <div className="overflow-hidden py-4" ref={emblaRef}>
      <div className="flex">
        {clients.map((client: any) => (
          <div
            key={client.id}
            className="flex-[0_0_50%] sm:flex-[0_0_25%] px-2"
          >
            <ClientLogo imageUrl={client.image} altText={client.alt} />
          </div>
        ))}
      </div>
    </div>
...
export default function ClientsSection() {
  ...
  return (
    <section className="py-8 sm:py-20 px-4 sm:px-8 md:px-12 lg:px-20">
      ...
      {clientChunks.map((chunk, idx) => (
        <ClientsRow key={idx} clients={chunk} />
      ))}
    </section>
  );
}
